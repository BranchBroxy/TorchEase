...
class Comparer(ModelBase):
    ...
    def evaluate_result(self):
        ...
        for i in range(self.n_models):
            self.evaluation_prediction = self.test_prediction.detach().cpu().numpy()[:, 0]
    ...
    def save_result(self, path_to_save="", CM_fname="ConfussionMatrix.png", barplot_fname="CompareBarPlot.png"):
        ...
        for i in range(self.n_models):
            ...
            _model_names.append(self.models[i].__class__.__name__)
            _confusion_matrices.append(self.confussion_martix)
        ...
        df = pd.DataFrame([_accuracy, _weighted_f1_score, _weighted_precision, _weighted_recall, _model_names], index=_columns_names)
        df_transposed = df.T
        ...
        plt.close()
        for index, cm in enumerate(_confusion_matrices):
